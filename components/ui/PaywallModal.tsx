...
export const PaywallModal: React.FC<PaywallModalProps> = ({ open, onClose, reason = 'game', gumroadUrl }) => {
    // Unmount immediately when closed. We deliberately DON'T use AnimatePresence here: with
    // this modal it failed to run the exit animation and left the dialog stuck on screen
    // (open=false but never unmounted) — i.e. "the popup won't close". A plain conditional
    // render guarantees the close. The enter animation still plays via initial/animate.
    if (!open) return null;
    ...
    return (
        <>
            ...
            <motion.div
                ...
                className="fixed inset-0 flex items-center justify-center z-[1000] p-4 pointer-events-none"
                dir="rtl"
            >
                <div className="relative bg-slate-900 border border-indigo-500/30 rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl shadow-indigo-500/20 font-cairo overflow-hidden pointer-events-auto">
                    {/* Glow */}
